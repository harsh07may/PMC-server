...
export async function searchApplications(
  ...
  try {
    ...
    const result = await pool.query(
      "SELECT * FROM application WHERE ref_id iLIKE '%' || $1 || '%' AND title iLIKE '%' || $2 || '%' AND holder iLIKE '%' || $3 || '%'",
      [ref_id, title, holder]
    );
