...
export async function searchApplications(
  ...
  try {
    const result = await pool.query(
      "SELECT DISTINCT a.* FROM application a JOIN application_trail t ON a.ref_id = t.ref_id WHERE a.ref_id ILIKE '%' || $1 || '%' AND a.title ILIKE '%' || $2 || '%' AND a.holder ILIKE '%' || $3 || '%' AND t.receiver ILIKE '%' || $4 || '%' AND t.sender ILIKE '%' || $5 || '%' AND a.outwarded::text ILIKE '%' || $6 || '%'",
      [ref_id, title, holder, receiver, sender, outwarded]
    );
...
export async function fetchTrailByStatus(receiver: string, status: string) {
  try {
    ...
    const result = await pool.query(
      "SELECT a.ref_id, a.title, a.created_at, a.outwarded, a.holder AS sender, at.transfer_time, at.trail_id FROM application a INNER JOIN application_trail at on at.ref_id=a.ref_id WHERE at.receiver = $1 and at.status = $2;",
      [receiver, status]
    );
    return result;
  } catch (error: any) {
...
export async function transferApplication(
  ...
  try {
    const result = await pool.query(
      "INSERT INTO application_trail(ref_id,transfer_no,sender,receiver) VALUES ($1,$2,$3,$4) RETURNING *",
...
export async function transferNewApplication(
  ...
  try {
    const result = await pool.query(
      "INSERT INTO application_trail(ref_id,transfer_no,transfer_time,sender,receiver,status) VALUES ($1,$2,(SELECT CURRENT_TIMESTAMP(0)::timestamp),$3,$4,'accepted') RETURNING *",
      [ref_id, transfer_no, sender, receiver]
    );
    return result;
  } catch (error: any) {
...
export async function OutwardApplication(ref_id: string) {
  try {
    await pool.query(
      "UPDATE application SET outwarded = true WHERE ref_id = $1",
      [ref_id]
    );
    console.log(ref_id);
  } catch (error: any) {
